...
| 9 | `open_only` cannot be expressed exactly | The engine's `GenerateOrderStatusReports.open_only` asks for **every non-terminal** order: `pending`, `open` and this adapter's `untriggered` are all working states | `GET /v1/perps/orders` filters by `status`, whose enum is exactly `open`, `canceled`, `fullyfilled` (`docs/api-reference/rest-spec.json`, sha256 `860a96ca…`) - there is no `pending` and no `untriggered` to ask for | The venue's own word for a working order is `open`, so `open_only: true` is sent as `status=open`: a faithful mapping of the venue's vocabulary, **not** an exact rendering of the flag. The residue is recorded rather than papered over - a `pending` or `untriggered` order is not reachable through a filtered history read, and what covers it is the unfiltered account read and the position judgment. The answer is never re-filtered locally: an order that arrives outside the requested status is reported, and named as a venue that did not apply the filter (`src/execution.rs`, the history read's `open_only` arm). |
